...
export function useAnnotationHighlighter({
  ...
  // Auto-apply persisted annotations that have no DOM marks (fixes Bug 2)
  useEffect(() => {
    const highlighter = highlighterRef.current;
    if (!highlighter || !containerRef.current || !annotations || annotations.length === 0) return;
    ...
    const needsRestore = annotations.filter(ann => {
      ...
      const manual = containerRef.current?.querySelector(`[data-bind-id="${ann.id}"]`);
      if (manual) return false;
      return true;
    });

    if (needsRestore.length > 0) {
      applyAnnotationsInternal(needsRestore);
    }
