...
if (args[0] === "sessions") {
  ...
  if (isPRMode) {
    ...
    if (!prRef) {
      ...
      console.error(
        "Supported formats: https://github.com/owner/repo/pull/123",
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(
        err instanceof Error ? err.message : "GitHub CLI auth check failed",
      );
      process.exit(1);
    }

    console.error(
      `Fetching PR #${prRef.number} from ${prRef.owner}/${prRef.repo}...`,
    );
    try {
  ...
  registerSession({
    ...
    label: isPRMode
      ? `pr-review-${prMetadata!.owner}/${prMetadata!.repo}#${prMetadata!.number}`
